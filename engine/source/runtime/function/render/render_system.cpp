...
void RenderSystem::processSwapData() {
    ...
    if (swap_data->dirty_game_objects.has_value()) {
        while (!swap_data->dirty_game_objects->empty()) {
            ...
            bool is_entity_in_scene =
                m_render_scene->entity_id_allocator.hasAsset(gobject.go_id);
            RenderEntity render_entity{};
            render_entity.entity_id =
                m_render_scene->entity_id_allocator.allocateGuid(gobject.go_id);
            render_entity.model_matrix = gobject.transform.matrix();

            MeshDesc mesh_desc = {gobject.mesh_desc.mesh_file};
            bool is_mesh_loaded = m_render_scene->mesh_guid_allocator.hasAsset(mesh_desc);
            MeshData mesh_data{};
            if (!is_mesh_loaded) {
                mesh_data =
    ...
    if (swap_data->dirty_game_objects.has_value()) {
        while (!swap_data->dirty_game_objects->empty()) {
            GameObjectDesc gobject = swap_data->dirty_game_objects->front();
            size_t guid;
            if (!m_render_scene->entity_id_allocator.getGuid(gobject.go_id, guid)) {
                continue;
            }
