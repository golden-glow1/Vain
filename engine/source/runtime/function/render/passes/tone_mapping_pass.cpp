...
static std::vector<uint8_t> s_tone_mapping_frag = {
#include "tone_mapping.frag.spv.h"
};

ToneMappingPass::~ToneMappingPass() { clear(); }

void ToneMappingPass::initialize(RenderPassInitInfo *init_info) {
    RenderPass::initialize(init_info);

    ToneMappingPassInitInfo *_init_info =
        static_cast<ToneMappingPassInitInfo *>(init_info);
    ...
    onResize(_init_info->input_attachment);
}
...
void ToneMappingPass::draw() {
    ...
    m_ctx->pushEvent(command_buffer, "Tone Map", color);
...
void ToneMappingPass::onResize(VkImageView input_attachment) {
    ...
    post_process_per_frame_input_attachment_info.imageLayout =
    ...
    post_process_descriptor_write.dstSet = descriptor_sets[0];
    post_process_descriptor_write.dstBinding = 0;
    post_process_descriptor_write.dstArrayElement = 0;
    post_process_descriptor_write.descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
    post_process_descriptor_write.descriptorCount = 1;
    post_process_descriptor_write.pImageInfo =
        &post_process_per_frame_input_attachment_info;

    vkUpdateDescriptorSets(m_ctx->device, 1, &post_process_descriptor_write, 0, nullptr);
}

void ToneMappingPass::createDecriptorSetLayouts() {
...
void ToneMappingPass::createPipelines() {
    pipelines.resize(1);
    ...
    VkShaderModule frag_shader_module =
        createShaderModule(m_ctx->device, s_tone_mapping_frag);
    ...
    pipeline_info.subpass = _subpass_tone_mapping;
...
void ToneMappingPass::allocateDecriptorSets() {
